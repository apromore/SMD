...
public class IncrementalDissimMatrixGenerator implements DissimilarityMatrix {
	
	private static final Logger logger = LoggerFactory.getLogger(IncrementalDissimMatrixGenerator.class);
...
//	}
	...
	public void computeDissimilarity(Collection<String> newRoots) throws Exception {
		
		// TODO: original minsize = 2
		crel.setMinSize(2);
		crel.initialize();
		
		setDissThreshold(MiningConfig.GED_THRESHOLD);
		addDissimCalc(new SizeBasedDissimCalc(dissThreshold));
		addDissimCalc(new GEDDissimCalc(dissThreshold, 0.6));
		...
		for (int p = 0; p < roots.size(); p++) {
			...
			if (!newRoots.contains(roots.get(p))) {
				continue;
			}
...
//				System.out.println(processedPairs + " processed out of " + totalPairs + " | " + percentage + " % completed. | Elapsed time: " + duration + " s | Distances to write: " + dissimmap.size());
	...
	public double compute(String frag1, String frag2) {
		...
		if (crel.getFragmentSize(frag1) > MiningConfig.MAX_GED_FRAGMENT_SIZE || 
				crel.getFragmentSize(frag2) > MiningConfig.MAX_GED_FRAGMENT_SIZE) {
			return disim;
		}
		
		// filter for small fragments
		if (crel.getFragmentSize(frag1) < MiningConfig.MIN_GED_FRAGMENT_SIZE || 
				crel.getFragmentSize(frag2) < MiningConfig.MIN_GED_FRAGMENT_SIZE) {
			return disim;
		}
