...
public class GTProcessStandardizer {
	
	private static final Logger logger = LoggerFactory.getLogger(GTProcessStandardizer.class);
	
	private Map<GTNode, CPF> standardizedProcesses = new HashMap<GTNode, CPF>();
	...
	@Autowired
	private GTClusterSelector clusterSelector;
	
	@Autowired
	private GTProcessImporter processModelImporter;
	...
	public void standardize() throws RepositoryException {
		...
		for (Process p : processes) {
			...
			if (rootfid != null) {
				CPF cpf = fsrv.getFragmentWithSubprocessesGT(rootfid, allSubprocesses);
				GTNode logCluster = processModelImporter.getNodeOfProcess(p.getName());
		...
		for (String subprocessId : stdSubprocesses) {
			...
			if (cpf == null) {
				cpf = fsrv.getFragmentWithSubprocessesGT(subprocessId, allSubprocesses);
			}
	 ...
	 */
	public Map<GTNode, CPF> getStandardizedProcesses() {
	...
	public void apply() {
		if (logger.isDebugEnabled()) {
			...
			for (GTNode logCluster : standardizedProcesses.keySet()) {
				try {
	...
	private void serializeStandardizedClusters() {
		...
		String outFilePath = "/home/cn/eps/bpm-demo-new1/bpm-demo/Apromore-Core/apromore-service/data/pm1/t1/gt/gtpcm/stdclusters.csv";
