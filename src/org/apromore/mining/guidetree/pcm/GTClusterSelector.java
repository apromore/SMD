...
public class GTClusterSelector {
	
	private final Logger logger = LoggerFactory.getLogger(GTClusterSelector.class);
...
//		if (complexityChecker.isComplex(mergedFragment)) { // TODO: without start and end
		if (mergedFragment.getVertices().size() > MiningConfig.COMPLEXITY_MATRIC_N) {
			GTEvaluatorUtil.incrementDiscardedMerges();
