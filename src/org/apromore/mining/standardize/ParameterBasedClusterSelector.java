...
public class ParameterBasedClusterSelector {
	
	private final Logger logger = LoggerFactory.getLogger(ParameterBasedClusterSelector.class);
...
//	private Map<String, CPF> mergedFragments = new HashMap<String, CPF>();
	...
	public Map<String, StandardizedCluster> getStandardizedClusters() {
		...
		// TODO: comment out the below for loop
//		for (String cid : cs.keySet()) {
//			
//			if (cid.startsWith("E")) {
...
//		}
	...
	private StandardizedCluster standardizeExactClone(String cid) {
		...
		logger.debug("Standardized the exact clone {} containing {} fragments and {} is selected as the representative fragment.", 
...
//		if (complexityChecker.isComplex(mergedFragment)) { // TODO: change
		if ((mergedFragment.getVertices().size()) > MiningConfig.COMPLEXITY_MATRIC_N) {
			EvaluatorUtil.incrementDiscardedMerges();
	...
	private StandardizedCluster standardizeWithMerge2(String cid) {

		List<String> fids = cs.get(cid);
		String mergedFragmentId = processMerger.merge(fids);
		CPF mergedFragment = processMerger.getMergedFragment(mergedFragmentId);
		if (complexityChecker.isComplex(mergedFragment)) {
			fids = getFragmentsOfComplexProcesses(fids);
			if (fids.size() < 2) {
				return null;
			}
			mergedFragmentId = processMerger.merge(fids);
			mergedFragment = processMerger.getMergedFragment(mergedFragmentId);
