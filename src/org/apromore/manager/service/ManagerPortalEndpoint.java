...
import org.apromore.model.GetProcessDataInputMsgType;
import org.apromore.model.GetProcessDataOutputMsgType;
...
public class ManagerPortalEndpoint {
    ...
    @Autowired @Qualifier("clusteringService")
    ...
    @PayloadRoot(namespace = NAMESPACE, localPart = "GetProcessDataRequest")
    @ResponsePayload
    public JAXBElement<GetProcessDataOutputMsgType> getProcessData(@RequestPayload final JAXBElement<GetProcessDataInputMsgType> req) {
        LOGGER.info("Executing operation getDataProcess");
        GetProcessDataInputMsgType payload = req.getValue();
        GetProcessDataOutputMsgType res = new GetProcessDataOutputMsgType();
        
        try {
	        String processId = payload.getProcessId();
	        String rootFragmentId = procSrv.getRootFragmentId(Integer.parseInt(processId));
	        res.setRootFragmentId(rootFragmentId);
        } catch (Exception e) {
        	LOGGER.error("Failed to get process data of the process " + payload.getProcessId(), e);
        	res.setRootFragmentId("ERROR");
        }
        return new ObjectFactory().createGetProcessDataResponse(res);
    }
    ...
    public JAXBElement<CreateClustersOutputMsgType> createClusters(@RequestPayload final JAXBElement<CreateClustersInputMsgType> req) {
        ...
        try {
			clusteringService.cluster(settings);
		} catch (RepositoryException e) {
    ...
    public JAXBElement<GetPairwiseDistancesOutputMsgType> getPairwiseDistances(@RequestPayload final JAXBElement<GetPairwiseDistancesInputMsgType> req) {
        ...
        try {
			Map<FragmentPair, Double> pairDistances = clusteringService.getPairDistances(fragmentIds);
    ...
    public JAXBElement<GetClusteringSummaryOutputMsgType> getClusteringSummary(@RequestPayload final JAXBElement<GetClusteringSummaryInputMsgType> req) {
        ...
        ClusteringSummary summary = clusteringService.getClusteringSummary();
    ...
    public JAXBElement<GetClusterSummariesOutputMsgType> getClusterSummaries(@RequestPayload final JAXBElement<GetClusterSummariesInputMsgType> req) {
        ...
        List<ClusterInfo> clusters = clusteringService.getClusterSummaries(filter);
    ...
    public JAXBElement<GetClusterOutputMsgType> getCluster(@RequestPayload final JAXBElement<GetClusterInputMsgType> req) {
        ...
        Cluster cluster = clusteringService.getCluster(clusterId);
//        postProcessCluster(cluster);
    ...
    public JAXBElement<GetClustersResponseType> getClusters(@RequestPayload final JAXBElement<GetClustersRequestType> req) {
        ...
        List<Cluster> clusters = clusteringService.getClusters(filter);
        for (Cluster c : clusters) {
//        	postProcessCluster(c);
    ...
    private void postProcessCluster(Cluster c) {
    	List<MemberFragment> tobeRemoved = new ArrayList<MemberFragment>();
//    	boolean centroidFound = false;
    	for (MemberFragment f: c.getFragments()) {
    		if (f.getDistance() == 0) {
    			tobeRemoved.add(f);
//    			if (!centroidFound) {
//    				centroidFound = true;
...
//    			}
    	...
    	c.getFragments().removeAll(tobeRemoved);
    	c.getClusterInfo().setSize(c.getClusterInfo().getSize() - tobeRemoved.size());
    }
