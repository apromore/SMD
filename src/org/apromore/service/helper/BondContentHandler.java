...
public class BondContentHandler {
	...
	public String matchFragment(RPSTNode f, Content matchingContent, Map<String, String> childMappings,
		...
		// TODO: properly fix the problem of content without enrty and/or exit nodes
		if (content.getSourceVertices().isEmpty() || content.getSinkVertices().isEmpty()) {
			return null;
		}
		...
		for (FragmentChildMapping fragmentChildMapping : candidateChildMappings) {
			List<FragmentVersionDagId> candidateMapping = fragmentChildMapping.getChildMapping();
			
			List<String> forwardCandidateChildIds = new ArrayList<String>();
			List<String> reverseCandidateChildIds = new ArrayList<String>();

			for (FragmentVersionDagId candidatePocket : candidateMapping) {
				if (forwardContentPocketIds.contains(candidatePocket.getPocketId())) {
					forwardCandidateChildIds.add(candidatePocket.getChildFragmentVersionId());
				}
				if (reverseContentPocketIds.contains(candidatePocket.getPocketId())) {
					reverseCandidateChildIds.add(candidatePocket.getChildFragmentVersionId());
				}
			}
	...
	private List<FragmentChildMapping> getCandidateChildMappings(String matchingContentId) {
		...
		for (String candidateFragmentId : candidateFragmentIds) {
			List<FragmentVersionDagId> childMapping = fvdDao.getChildMappings(candidateFragmentId);
